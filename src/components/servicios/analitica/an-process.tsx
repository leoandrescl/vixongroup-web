...
import { anProcessSteps } from "@/content/analitica";
...
const stepIcons = {
  ...
  cog: Settings,
  rocket: Rocket,
} as const satisfies Record<(typeof anProcessSteps)[number]["icon"], LucideIcon>;

export function AnProcess() {
  return (
    <Section className="scroll-mt-24 bg-white text-canvas-foreground">
      <Container>
        ...
        <ol className="mt-10 grid grid-cols-2 gap-8 overflow-visible lg:grid-cols-6 lg:gap-0">
          {anProcessSteps.map((step, index) => {
            const Icon = stepIcons[step.icon];
            const isLast = index === anProcessSteps.length - 1;
