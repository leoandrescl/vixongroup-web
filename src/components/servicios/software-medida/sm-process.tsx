...
import { smProcessSteps } from "@/content/software-medida";
...
const stepIcons = {
  ...
  cog: Settings,
  rocket: Rocket,
} as const satisfies Record<(typeof smProcessSteps)[number]["icon"], LucideIcon>;
    ...
    <Section className="scroll-mt-24 bg-white text-canvas-foreground">
      <Container>
        ...
        <ol className="mt-10 grid grid-cols-2 gap-8 overflow-visible lg:grid-cols-6 lg:gap-0">
          {smProcessSteps.map((step, index) => {
            const Icon = stepIcons[step.icon];
            const isLast = index === smProcessSteps.length - 1;
