...
import { cgProjectTypes } from "@/content/contenido";

export function CgProjectTypes() {
  return (
    <Section className="scroll-mt-24 bg-[#e8f7fb] text-canvas-foreground">
      <Container>
        <Reveal>
          <Eyebrow className="text-[#00c8ea]">{cgProjectTypes.eyebrow}</Eyebrow>
          <h2 className="mt-3 max-w-2xl text-3xl font-semibold tracking-tight text-balance md:text-4xl lg:text-[2.35rem] lg:leading-[1.12]">
            {cgProjectTypes.titleBefore}{" "}
            <span className="text-[#00c8ea]">{cgProjectTypes.titleAccent}</span>
          </h2>
        </Reveal>

        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 lg:gap-4">
          {cgProjectTypes.items.map((item, index) => (
