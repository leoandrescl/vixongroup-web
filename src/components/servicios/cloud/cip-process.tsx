...
import { cipProcessSteps } from "@/content/cloud";
...
const stepIcons = {
  ...
  cog: Settings,
  rocket: Rocket,
} as const satisfies Record<(typeof cipProcessSteps)[number]["icon"], LucideIcon>;

export function CipProcess() {
  return (
    <Section className="scroll-mt-24 bg-white text-canvas-foreground">
      <Container>
        ...
        <ol className="mt-10 grid grid-cols-2 gap-8 overflow-visible lg:grid-cols-6 lg:gap-0">
          {cipProcessSteps.map((step, index) => {
            const Icon = stepIcons[step.icon];
            const isLast = index === cipProcessSteps.length - 1;
