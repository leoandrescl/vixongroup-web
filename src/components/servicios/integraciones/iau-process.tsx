...
import { iauProcessSteps } from "@/content/integraciones";
...
const stepIcons = {
  ...
  cog: Settings,
  rocket: Rocket,
} as const satisfies Record<(typeof iauProcessSteps)[number]["icon"], LucideIcon>;

export function IauProcess() {
  return (
    <Section className="scroll-mt-24 bg-white text-canvas-foreground">
      <Container>
        ...
        <ol className="mt-10 grid grid-cols-2 gap-8 overflow-visible lg:grid-cols-6 lg:gap-0">
          {iauProcessSteps.map((step, index) => {
            const Icon = stepIcons[step.icon];
            const isLast = index === iauProcessSteps.length - 1;
